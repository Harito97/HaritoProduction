...
class ThyroidCancerClassificationModel:
    ...
    def get_output_model2(self, images):
        ...
        output_model2.backward(
            gradient=output_model2
        )  # torch.ones_like(output_model2))

        ## Here we calculate the CAM in the end of CNNN
        ###############################################
        ...
        feature_maps_reshaped = feature_maps.reshape(
            18, num_channels, H * W
        )  # (num_channels, H*W)
        cam = torch.matmul(weights, feature_maps_reshaped)  # (H*W)
        print(cam.shape)
        print(feature_maps_reshaped.shape)
        cam = cam.view(
            feature_maps.size(0), feature_maps.size(2), feature_maps.size(3)
        )  # (H, W)
        print(cam.shape)
        cam_end_CNN_softmax = F.softmax(cam, dim=0)
        print(cam_end_CNN_softmax.shape)

        ## Here we calculate the CAM in the begin of CNNN
        #################################################
        ...
        # print(cam.shape)
        # cam_begin_CNN_softmax = torch.mean(cam, dim=1)
        # cam_begin_CNN_softmax = F.softmax(cam_begin_CNN_softmax, dim=1)
        # print(cam_begin_CNN_softmax.shape, cam_begin_CNN_softmax.min(), cam_begin_CNN_softmax.max())
    ...
    def forward(self, image_origin_dir):
        ...
        output_model2, cam_images_end_CNN = self.get_output_model2(
            output_model1
        )  # cam_images_begin_CNN
        output_model3 = self.get_output_model3(output_model2)
        _, predicted = torch.max(output_model3, 1)
        return (
            predicted,
            (output_model3, output_model2, output_model1),
            cam_images_end_CNN,
        )  # cam_images_begin_CNN

    def run_app(self, image_origin_dir):
        predicted, outputs, cam_images_end_CNN = self.forward(image_origin_dir)
        # Ghép các phần lại với nhau
        top_row = np.hstack(
            (
                cam_images_end_CNN[0],
                cam_images_end_CNN[1],
                cam_images_end_CNN[2],
                cam_images_end_CNN[3],
            )
        )
        middle_row = np.hstack(
            (
                cam_images_end_CNN[4],
                cam_images_end_CNN[5],
                cam_images_end_CNN[6],
                cam_images_end_CNN[7],
            )
        )
        bottom_row = np.hstack(
            (
                cam_images_end_CNN[8],
                cam_images_end_CNN[9],
                cam_images_end_CNN[10],
                cam_images_end_CNN[11],
            )
        )
        # Ghép các hàng lại với nhau để tạo thành ảnh tổng hợp
        patch_level_heatmap = cv2.resize(
            np.vstack((top_row, middle_row, bottom_row)), (1024, 768)
        )
        # Image level heatmap
        image_level_heatmap = cv2.resize(
            cam_images_end_CNN[17].detach().numpy(), (1024, 768)
        )
        # YOLO detect image
