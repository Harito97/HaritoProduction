...
class H97_ThyroidCancer:
    ...
    def overlay_cam_on_image(self, image, cam, alpha=0.5, colormap="jet"):
        """
        ...
        blended_image = np.clip(
            alpha * cam_color + (1 - alpha) * new_image, 0, 255
        ).astype(np.uint8)

        return blended_image

    def main(self, image_dir, alpha=0.5):
        (
            predicted,
            outputs,
            patch_level_heatmap,
            image_level_heatmap,
            yolo_detect_image,
        ) = self.run_app(image_dir)
        patch_level_heatmap = self.overlay_cam_on_image(
            self.origin_image, patch_level_heatmap, alpha=alpha
        )
        image_level_heatmap = self.overlay_cam_on_image(
            self.origin_image, image_level_heatmap, alpha=alpha
        )
        return (
            predicted,
            outputs[0],
    ...
    def get_images(self, image_origin_dir=None):
        ...
        self.yolo_detect_image = cv2.cvtColor(self.yolo_detect_image, cv2.COLOR_BGR2RGB)
